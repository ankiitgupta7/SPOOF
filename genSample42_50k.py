...
OUTPUT_JSON = "imagenet_val_all_correct_1000_new.json"
...
# ----- MAIN PIPELINE -----
def main():
    print(">>> Full evaluation of all 50,000 validation images started")
    ...
    for img_file in tqdm(all_image_files, desc="Evaluating all"):
    ...
    print(f"\n✅ Correctly predicted by all models: {len(all_results)} / {total_images}")

    # Print accuracy per model
    print("\n🔍 Top-1 Accuracy per model:")
    for name in correct_counts:
        acc = correct_counts[name] / total_images * 100
        print(f"{name}: {correct_counts[name]} / {total_images} = {acc:.2f}%")

    # Sample 1000 from the correct set
    random.seed(SEED)
    ...
    print(f"\n📁 Saved {len(final_sample)} entries to {OUTPUT_JSON}")
